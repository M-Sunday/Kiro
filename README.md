...
- **External files** — Import images, videos, and text files from your device. Grid view with thumbnails (base64 data URLs for small images, `file://` for large, canvas-captured frames for videos). In-app text viewer, image viewer, and video player with full custom controls. Assign to folders, blur/unblur, drag-to-reorder sidebar. Files appear under their assigned folders in the sidebar tree. Re-import stale files via native file picker on Android/iOS (Capacitor).
